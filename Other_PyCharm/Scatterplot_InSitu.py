...
# Output directory
OUTPUT_DIR = BASE_DIR / "Scatter_Obs_vs_Sim" / "In-situ"
...
def plot_scatter_obs_vs_sim(df, group, station_short, obs_col, vic_col=None, mgv_col=None, sat_col=None):
    ...
    - y = VIC, mGV, and satellite SM (if available)
    """
    ...
    if vic_col and vic_col in df.columns:
        sim_datasets["VIC"] = (vic_col, "#1f77b4")
    if mgv_col and mgv_col in df.columns:
        sim_datasets["mGV"] = (mgv_col, "#ff7f0e")
    if sat_col and sat_col in df.columns and group == "SM":
        sim_datasets["Satellite"] = (sat_col, "#9467bd")  # purple-ish for satellite
    ...
    # ─── Style & labels ───────────────────────────────
    ax.set_xlabel(f"In-situ (observed) – {full_ylabel}", fontsize=12)
    ax.set_ylabel(f"Simulated – {full_ylabel}", fontsize=12)

    ax.set_title(f"{full_station} – {full_var_name}", fontsize=14, fontweight='bold', pad=16)
...
def main():
    ...
    for station_short in STATION_DISPLAY_NAMES.keys():
        ...
        # ─── Add satellite SM if available and group is SM ────────────────
        sat_col = None
        if station_short in SAT_SM_COLS and xl_sat_sm is not None:
            ...
            if sat_sheet in xl_sat_sm.sheet_names:
                ...
                # Join satellite SM to the main DataFrame
        ...
        for group, insitu_col in INSITU_VARS.get(station_short, []):
            ...
            # For SM, pass satellite column if available
            current_sat_col = sat_col if group == "SM" else None

            # Plot if we have in-situ + at least one simulated dataset

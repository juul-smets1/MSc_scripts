...
OUTPUT_DIR = BASE_DIR / "Scatter_Obs_vs_Sim" / "Final"
...
def plot_scatter_obs_vs_sim(df, var_info, region_sheet):
    ...
    # Scatter points
    ax.scatter(obs, vic, s=25, alpha=0.5, color="#1f77b4", label="VIC-WUR", edgecolor="none")
    ax.scatter(obs, mgv, s=25, alpha=0.5, color="#ff7f0e", label="VIC-Julia", edgecolor="none")

    # 1:1 reference line
    min_val = min(obs.min(), vic.min(), mgv.min()) * 0.95
    max_val = max(obs.max(), vic.max(), mgv.max()) * 1.05
    ax.plot([min_val, max_val], [min_val, max_val], color='black', lw=1.8, linestyle='--', alpha=0.7, label='1 : 1 line')

    # ─── Style & labels with requested font sizes ───────────────────────────────
    ax.set_xlabel(f"Satellite (observed) – {full_ylabel}", fontsize=20)
    ax.set_ylabel(f"Simulated – {full_ylabel}", fontsize=20)

    # Title: size 25, NOT bold
    ax.set_title(f"{full_region} – {full_var_name}", fontsize=25, pad=16)
...
def main():
    ...
    for sheet_g, sheet_s in REGION_MAPPING.items():
        ...
        for var_info in VARIABLE_GROUPS:
            ...
            if not all(c in df_sim.columns for c in [c_vic, c_mgv]) or c_obs not in df_obs.columns:

...
# Model columns per station (VIC + mGV outputs)
MODEL_COLS = {
...
# Output directory
OUTPUT_DIR = BASE_DIR / "Performance_Plots" / "In-situ" / "Final"
...
def plot_triple_timeseries(df, group, station_short, obs_col, vic_col=None, mgv_col=None, sat_col=None):
    ...
    # Count how many lines will be plotted (to set legend ncols dynamically)
    n_legend_items = 1  # always has in-situ

    # Always plot in-situ observation (reference line)
    ax.plot(df.index, df[obs_col], label=f"In-situ", lw=1.4, color="#2ca02c",
            alpha=0.9, linestyle="--")

    # Plot VIC-WUR if available
    if vic_col and vic_col in df.columns:
        ax.plot(df.index, df[vic_col], label=f"VIC-WUR", lw=1.1, color="#1f77b4")
        n_legend_items += 1

    # Plot VIC-Julia if available
    if mgv_col and mgv_col in df.columns:
        ax.plot(df.index, df[mgv_col], label=f"VIC-Julia", lw=1.1, color="#ff7f0e")
        n_legend_items += 1

    # Overlay satellite SM if provided (solid line, purple-ish for distinction)
    if sat_col and sat_col in df.columns and group == "SM":
        ax.plot(df.index, df[sat_col], label=f"Satellite", lw=1.4, color="#9467bd",
                alpha=0.9, linestyle="-")
        n_legend_items += 1

    ax.set_title(f"{full_station_name} — {full_var_name}", fontsize=25, pad=12)
    ...
    # Legend placed below the x-axis; ncols matches the number of plotted lines
    ax.legend(loc="upper center", bbox_to_anchor=(0.5, -0.12), ncol=n_legend_items,
              fontsize=17, framealpha=0.92)
...
def main():
    ...
    for station_short in STATION_DISPLAY_NAMES.keys():
        ...
        for group, insitu_col in INSITU_VARS.get(station_short, []):
            ...
            # Plot the full comparison (in-situ + VIC-WUR + VIC-Julia)
